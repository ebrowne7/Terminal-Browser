  ...
  toJSON() {
    const serializeNode = (node) => {
      ...
      node.children.forEach((child) => {
        childrenArr.push(serializeNode(child));
      });
      return { type: 'dir', name: node.name, children: childrenArr };
    };
    return serializeNode(this.root);
  }

  fromJSON(data) {
    const deserializeNode = (dataNode) => {
      if (dataNode.type === 'file') {
        return { type: 'file', name: dataNode.name, content: dataNode.content, isEncrypted: dataNode.isEncrypted };
      }
      const childrenMap = new Map();
      if (dataNode.children) {
        dataNode.children.forEach((childData) => {
          childrenMap.set(childData.name, deserializeNode(childData));
        });
      }
      return { type: 'dir', name: dataNode.name, children: childrenMap };
    };
...
class MissionManager {
  ...
  initObjectives() {
    this.objectives = [
      { id: 'probe_bank', description: 'Probe open ports on Bank Central Gateway (192.168.1.104)', completed: false },
      { id: 'crack_pass', description: 'Crack /sys/passwords.enc on Bank Central Gateway', completed: false },
      { id: 'download_db', description: 'Download accounts.db from Bank Central Gateway', completed: false },
      { id: 'probe_military', description: 'Probe open ports on Defense Grid Node (10.0.99.1)', completed: false },
      { id: 'crack_payload', description: 'Crack and retrieve missile_schematics.enc on Defense Grid Node', completed: false },
      { id: 'clear_logs', description: 'Delete /var/logs/access.log on Defense Grid Node', completed: false }
    ];
  }
  ...
  completeObjective(id) {
    ...
    if (obj && !obj.completed) {
      obj.completed = true;
      saveGameState(true); // Silent auto-save on milestone completion
      return true;
    }
    return false;
  }
...
// --- 3. Network Registry & Default Nodes ---
let bounceChain = [];
let networkNodes = {};
...
function initNetworkNodes() {
  bounceChain = [];
  networkNodes = {
    "127.0.0.1": { hostname: "LOCALHOST", baseTraceRate: 0, isProxyable: false, ports: [22, 80], vfs: new VirtualFileSystem() },
    "10.0.8.2": { hostname: "OPEN-PROXY-US", baseTraceRate: 0, isProxyable: true, ports: [80, 8080, 3128], vfs: new VirtualFileSystem() },
    "10.0.8.9": { hostname: "OPEN-PROXY-EU", baseTraceRate: 0, isProxyable: true, ports: [80, 1080], vfs: new VirtualFileSystem() },
    "192.168.1.104": { hostname: "BANK-CENTRAL-GATEWAY", baseTraceRate: 3.5, isProxyable: false, ports: [21, 22, 80, 443], vfs: new VirtualFileSystem() },
    "10.0.99.1": { hostname: "DEFENSE-GRID-NODE", baseTraceRate: 9.0, isProxyable: false, ports: [22, 8080, 9001], vfs: new VirtualFileSystem() }
  };
...
function saveGameState(silent = false) {
  try {
    const serializedVFS = {};
    for (const ip in networkNodes) {
      serializedVFS[ip] = networkNodes[ip].vfs.toJSON();
    }

    const state = {
      objectives: missions.objectives,
      filesystems: serializedVFS,
      bounceChain: bounceChain,
      currentIp: traceEngine.currentIp
    };

    localStorage.setItem('terminal_game_save', JSON.stringify(state));
    if (!silent) ui.print("[SYSTEM] Progress saved successfully to local storage.", "warn");
  } catch (err) {
    ui.print(`[ERROR] Failed to save state: ${err.message}`, "alert");
  }
}
...
function loadGameState() {
  ...
  try {
    ...
    missions.objectives = state.objectives;

    for (const ip in state.filesystems) {
      if (networkNodes[ip]) {
        networkNodes[ip].vfs.fromJSON(state.filesystems[ip]);
      }
    }
    ...
    if (state.currentIp && state.currentIp !== "127.0.0.1") {
      traceEngine.connect(state.currentIp);
    }
...
const commands = {
  ...
  save: async () => saveGameState(false),
...
// Boot Banner & Initialization
const loaded = loadGameState();
ui.print("NODE-OS Terminal System v1.09");
if (loaded) {
  ui.print("[RESTORED] Loaded active state from local storage.");
}
ui.print("Type 'objectives' to list contract targets.");
ui.print("Type 'cat home/user/bounces.txt' to discover proxy IPs.\n");
ui.setPrompt(traceEngine.currentIp, traceEngine.getActiveVFS().getAbsolutePath());